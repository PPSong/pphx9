angular.module('starter.controllers', ['angularMoment', 'timer'])
  .controller('starterCtrl', function($scope, $ionicLoading, PPConsole) {
    ...
    var serverUrl = "192.168.1.4:3000";
    ...
    $scope.unreadMessageCounts = $scope.asteroid.getCollection(
      "unreadMessageCounts");
  ...
  .controller('LoginCtrl', function($scope, $state, PPConsole) {
    ...
    $scope.login = function() {
      $scope.asteroid.loginWithPassword($scope.loginUser.username, $scope.loginUser
  ...
  .controller('RegisterCtrl', function($scope, $state, $ionicLoading,
    OptionService, PPConsole) {
    $scope.registerUser = {};
    ...
    $scope.register = function() {
      ...
      $scope.asteroid.createUser($scope.registerUser.username, $scope.registerUser
        .password, {
  ...
  .controller('MeetCtrl', function($scope, $state, $ionicModal, $ionicLoading,
    $ionicPopup, $cordovaCamera, $cordovaGeolocation, OptionService,
    BaiduNearByLocationService, PPConsole) {
    ...
    $scope.chooseOptionPlace = function(object) {
      OptionService.initOptionWithInput(object, 'place', '场所', $scope.usersRQ
        .result[0].profile.lastLocation);
    };
    ...
    $scope.chooseOptionHair = function(object, sex) {
      if (sex == "男") {
        OptionService.initOption(object, ['(男)竖起来(包括光头)', '(男)躺下', '(男)戴帽子'],
          'hair', '发型');
      } else {
        OptionService.initOption(object, ['(女)辫子/盘发', '(女)短发(齐肩,不过肩)',
          '(女)长发(过肩)', '(女)戴帽子'
        ], 'hair', '发型');
      }
    };
    ...
    $scope.chooseOptionClothesType = function(object, sex) {
      if (sex == "男") {
        OptionService.initOption(object, ['(男)风衣/大衣', '(男)西装/夹克/套装',
          '(男)运动外套/卫衣', '(男)T恤长袖', '(男)T恤短袖', '(男)马甲/背心', '(男)长袖衬衫',
          '(男)短袖衬衫', '(男)毛衣/羊毛绒/线衫/针织'
        ], 'clothesType', '衣服类型');
      } else {
        OptionService.initOption(object, ['(女)风衣/大衣', '(女)西装/夹克/套装',
          '(女)运动外套/卫衣', '(女)T恤长袖', '(女)T恤短袖', '(女)马甲/背心', '(女)长袖衬衫',
          '(女)短袖衬衫', '(女)毛衣/羊毛绒/线衫/针织', '(女)连体裙'
        ], 'clothesType', '衣服类型');
      }
    };
    ...
    $scope.chooseOptionClothesColor = function(object, sex) {
      if (sex == "男") {
        OptionService.initOption(object, ['(男)红/紫/粉', '(男)黄', '(男)蓝/绿',
          '(男)白', '(男)黑', '(男)灰', '(男)彩色,且难以判断主体颜色'
        ], 'clothesColor', '衣服颜色');
      } else {
        OptionService.initOption(object, ['(女)红/紫/粉', '(女)黄', '(女)蓝/绿',
          '(女)白', '(女)黑', '(女)灰', '(女)彩色,且难以判断主体颜色'
        ], 'clothesColor', '衣服颜色');
      }
    };
    ...
    $scope.chooseOptionClothesStyle = function(object, sex) {
      if (sex == "男") {
        OptionService.initOption(object, ['(男)纯色', '(男)线条,格子,色块',
          '(男)图案(抽象,卡通,画等有具体内容)'
        ], 'clothesStyle', '衣服花纹');
      } else {
        OptionService.initOption(object, ['(女)纯色', '(女)线条,格子,色块',
          '(女)图案(抽象,卡通,画等有具体内容)'
        ], 'clothesStyle', '衣服花纹');
      }
    };
    ...
    $scope.popupBigPic = function() {
      var alertPopup = $ionicPopup.alert({
        ...
        template: ' <img class="pp-special-pic-big" src="' + ($scope.editingSpecialInfo
          .specialPic) + '">'
      });
    };
    ...
    $scope.createMeet = function() {
      ...
      $cordovaGeolocation.getCurrentPosition(posOptions).then(function(
        ...
        tmpPromiseResult.result.then(function(r) {
          ...
          $scope.targetSpecialInfo.data.sex = ($scope.usersRQ.result[
            0].profile.sex === '男' ? '女' : '男');
    ...
    $scope.searchCreateTarget = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("createMeetSearchTarget",
        $scope.targetSpecialInfo.data);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
    ...
    $scope.clickMeet = function(item) {
      ...
      if (item.createrUserId === $scope.usersRQ.result[0]._id) {
        if (item.status === "待确认") {
          ...
          var tmpPromiseResult = $scope.asteroid.call(
            "createMeetSearchTarget", $scope.targetSpecialInfo.data);
          tmpPromiseResult.result.then(function(r) {
            PPConsole.debug("rr");
            PPConsole.debug(r);
            $scope.targets.data = r;
    ...
    $scope.searchReplyTarget = function() {
      if (!(
          $scope.targetSpecialInfo.data.sex && $scope.targetSpecialInfo.data
          .clothesColor && $scope.targetSpecialInfo.data.clothesStyle &&
          $scope.targetSpecialInfo.data.clothesType && $scope.targetSpecialInfo
          .data.glasses && $scope.targetSpecialInfo.data.hair
        )) {
      ...
      var tmpPromiseResult = $scope.asteroid.call("replyMeetSearchTarget",
        $scope.curMeet.data._id, $scope.targetSpecialInfo.data);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
        if (r == '特征信息不匹配') {
    ...
    $scope.saveSpecialInfo = function() {
      ...
      $cordovaGeolocation.getCurrentPosition(posOptions).then(function(
        ...
        var tmpPromiseResult = $scope.asteroid.call(
          "saveSpecialInfoAndPosition", $scope.editingSpecialInfo, [
            long, lat
          ]);
        tmpPromiseResult.result.then(function(r) {
          PPConsole.debug("rr");
    ...
    $scope.takePhoto = function() {
      if (typeof Camera == "undefined") { //不是手机, 是浏览器, 测试用
        $scope.editingSpecialInfo.specialPic =
  ...
  .controller('SearchSpecialPicCtrl', function($scope, $ionicModal, $state,
    ...
    $scope.selectSuccess = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("createMeetChooseTarget",
        $scope.curTarget.data._id, $scope.targetSpecialInfo.data);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
        $state.go("tab.meet");
    ...
    $scope.createNeedConfirm = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("createNeedConfirm",
        $scope.targetSpecialInfo.data);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
  ...
  .controller('SearchSpecialPicConfirmCtrl', function($scope, $ionicModal,
    $state, $ionicLoading, PPConsole) {
    $ionicModal.fromTemplateUrl('templates/modalBigSpecialPic.html', {
      scope: $scope,
      animation: 'slide-in-up'
    }).then(function(modal) {
    ...
    $scope.selectSuccess = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("confirmMeetChooseTarget",
        $scope.curMeet.data._id, $scope.curTarget.data._id);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
        $scope.modalBigSpecialPic.hide();
    ...
    $scope.noTarget = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("clearNewMatchCount",
        $scope.curMeet.data._id);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
        $state.go("tab.meet");
  ...
  .controller('SearchSpecialPicReplyCtrl', function($scope, $ionicModal, $state,
    ...
    $scope.selectSuccess = function() {
      if ($scope.curTarget.data.userId == 'fake') {
        ...
        var tmpPromiseResult = $scope.asteroid.call("replyMeetClickTarget",
          $scope.curMeet.data._id, $scope.curTarget.data.userId);
        tmpPromiseResult.result.then(function(r) {
          PPConsole.debug("rr");
          PPConsole.debug(r);
  ...
  .controller('ActivityCtrl', function($scope, $state, $ionicModal,
    $ionicLoading, PPConsole) {
    ...
    $scope.chooseMark = function(mark) {
      ...
      var tmpPromiseResult = $scope.asteroid.call("chooseMark", $scope.joiningActivity
        ._id, mark);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
  ...
  .controller('FriendCtrl', function($scope, $state) {
    ...
    $scope.goToChat = function(item) {
      $state.go('tab.friend.chat', {
        friendUserId: $scope.usersRQ.result[0]._id == item.userId1 ?
          item.userId2 : item.userId1
      });
    };
  })
  .controller('ChatCtrl', function($scope, $state, $stateParams,
    $ionicScrollDelegate, $timeout, PPConsole) {
    ...
    $scope.sendMessage = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("sendMessage", $scope.inputMessage,
        $scope.friendUserId);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
  ...
  .controller('ActivityChatCtrl', function($scope, $state, $stateParams,
    $ionicScrollDelegate, $ionicModal, $timeout, PPConsole) {
    $scope.activityId = $stateParams.activityId;

    $scope.historyMessage = [];
    $scope.historyMessageTimeStamp;

    var tmpPromiseResult = $scope.asteroid.call("getGroupMessageTimeStamp",
      $scope.activityId);
    tmpPromiseResult.result.then(function(r) {
      PPConsole.debug("rr");
      PPConsole.debug(r);
      $scope.historyMessageTimeStamp = r;
      $scope.groupChatMessagesHandle = $scope.asteroid.subscribe(
        "groupChatMessages", $scope.activityId, r);
      $scope.groupChatMessages = $scope.asteroid.getCollection(
        "groupChatMessages");
    ...
    $scope.getHistoryMessage = function() {
      var tmpPromiseResult = $scope.asteroid.call("getHistoryMessage",
        $scope.activityId, $scope.historyMessageTimeStamp);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
    ...
    $scope.deChooseLike = function(targetUserId) {
      var tmpPromiseResult = $scope.asteroid.call("deChooseLike", $scope.activityId,
        targetUserId);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
    ...
    $scope.chooseLike = function(targetUserId) {
      var tmpPromiseResult = $scope.asteroid.call("chooseLike", $scope.activityId,
        targetUserId);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
    ...
    $scope.sendMessage = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("sendGroupMessage",
        $scope.inputMessage, $scope.activityId);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
        $scope.inputMessage = '';
    ...
    $scope.myBack = function() {
      var tmpPromiseResult = $scope.asteroid.call("readGroupMessage",
        $scope.activityId);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
  ...
  .controller('SettingCtrl', function($scope, $state, $ionicLoading,
    $cordovaGeolocation, PPConsole) {
    $scope.friend = {
      username: ''
    };
    $scope.addFriend = function() {
      ...
      var tmpPromiseResult = $scope.asteroid.call("addFriend", $scope.friend
        .username);
      tmpPromiseResult.result.then(function(r) {
        PPConsole.debug("rr");
        PPConsole.debug(r);
