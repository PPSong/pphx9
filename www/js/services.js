angular.module('starter.services', ['ngCordova'])
  .factory('PPConsole', function($cordovaToast) {
    return {
      err: function(obj) {
        try {
          $cordovaToast.show("[PPError:]" + obj.message || obj ||
            "err occur!", 'long', 'center');
        } catch (e) {
          console.log("[PPError:]");
          console.log(obj);
        }
      ...
      debug: function(obj) {
        if (false) {
          try {
            $cordovaToast.show("[PPDebug:]" + obj.message || obj ||
              "err occur!", 'long', 'bottom');
          } catch (e) {
            console.log("[PPDebug:]");
            console.log(obj);
          }
      ...
      show: function(obj) {
        try {
          $cordovaToast.show(obj.message || obj || "err occur!", 'long',
            'center');
        } catch (e) {
          console.log("[PPShow:]");
          console.log(obj);
  ...
  .factory("BaiduNearByLocationService", function($http, $q) {
    return {
      getLocations: function(keyword, lng, lat) {
        ...
        var convertUrl =
          "http://api.map.baidu.com/geoconv/v1/?callback=JSON_CALLBACK&";
        var convertData = "ak=" + ak + "&coords=" + lng + "," + lat;
        var getPlaceUrl =
          "http://api.map.baidu.com/place/v2/search?callback=JSON_CALLBACK&";
        var getPlaceDate = "ak=" + ak +
          "&output=json&radius=2000&scope=1&filter=sort_name:distance&query=" +
          keyword;

        $http.jsonp(convertUrl + convertData).then(
          function(r) {
            var tmpLocation = "&location=" + r.data.result[0].y + "," + r
              .data.result[0].x;
  ...
  .factory("OptionService", function($rootScope, $ionicPopup,
    BaiduNearByLocationService) {
    return {
      ...
      initOptionWithInput: function(object, itemName, title, curLocation) {
        ...
        thisScope.keywordSearch = function(keyword) {
          BaiduNearByLocationService.getLocations(keyword, curLocation[0],
            curLocation[1]).then(function(options) {
            thisScope.options = options;
          }, function(e) {
            PPConsole.err(e);
            this.Scope.options = [];
